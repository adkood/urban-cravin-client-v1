...
export default function WaterRippleEffect({
  ...
  useEffect(() => {
    ...
    const initialRect = mountElement.getBoundingClientRect()
    const targetWidth = fitToContainer ? Math.max(1, Math.floor(initialRect.width)) : width
    const targetHeight = fitToContainer ? Math.max(1, Math.floor(initialRect.height)) : height
    ...
    const handleResize = () => {
      ...
      const newWidth = fitToContainer ? Math.max(1, Math.floor(rect.width)) : width
      const newHeight = fitToContainer ? Math.max(1, Math.floor(rect.height)) : height
      ...
      if (geometryRef.current && meshRef.current) {
        ...
        const newGeometry = new THREE.PlaneGeometry(4 * newAspect, 4, 64, 64)
    ...
    if (fitToContainer) {
      ...
      // Resize once in case fonts/layout shift after mount
    ...
    imageSrc, width, height, fitToContainer, waveIntensity, rippleIntensity, animationSpeed, hoverRippleMultiplier, transitionSpeed,
